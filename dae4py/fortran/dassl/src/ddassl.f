...
C   250124  Replaced required LINPACK functions by corresponding LAPACK 
C           calls. (J. Breuling, INM Stuttgart)
C           (i) banded matrices: DGBFA, DGBSL => DGBTRF, DGBTRS
C           (ii) dense matrices: DGEFA, DGESL => DGETRF, DGETRS
...
550   CALL DGBTRF(NEQ, NEQ, IWM(LML), IWM(LMU), WM(NPD), 
     *   MEBAND, IWM(LIPVT), IER)
      RETURN
...
C
      INTEGER  LIPVT, LML, LMU, LMTYPE, MEBAND, MTYPE, NPD, IER
      PARAMETER (NPD=1)
      PARAMETER (LML=1)
      PARAMETER (LMU=2)
      PARAMETER (LMTYPE=4)
...
C
C     DENSE MATRIX
100   CALL DGETRS('N', NEQ, 1, WM(NPD), NEQ, IWM(LIPVT), 
     *   DELTA, NEQ, IER)
      RETURN
C
...
C
C     BANDED MATRIX
400   CALL DGBTRS('N', NEQ, IWM(LML), IWM(LMU), 1, WM(NPD), MEBAND, 
