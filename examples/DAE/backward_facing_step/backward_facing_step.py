...
see Harlow1965, Jovic1994 and Le1997. The velocity flow profiles are found 
in cfd.mace.manchester.
...
def redundant_coordinates(y, yp, nx, ny, BC):
    ...
    # boundary conditions
    apply_boundary_conditions(u_red, v_red, p_red, BC)
...
def F(t, y, yp, eps=1e-12):
    ...
    u, v, p, ut, vt, pt = redundant_coordinates(y, yp, nx, ny, BC)
...
if __name__ == "__main__":
    ...
    u_left_profile = np.interp(y, y_ref, u_ref)

    fig, ax = plt.subplots(2, 1)

    ax[0].title.set_text(f"x / h: {xh}")
    ax[0].plot(u_ref, y_ref, "-", label="u_ref")
    ax[0].plot(u_left_profile, y, "--o", label="u_ref interp")
    ...
    for i in range(nt):
        u[:, :, i], v[:, :, i], p[:, :, i], ut[:, :, i], vt[:, :, i], pt[:, :, i] = (
            redundant_coordinates(sol_y[:, i], sol_yp[:, i], nx, ny, BC)
        )
