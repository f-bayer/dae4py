...
def generate_system(Lx, Ly, nx, ny, nu, BC):
    ...
    def F(t, y, yp):
        ...
        Fu = (
            ut[1:-1, 1:-1]
            + (u2ij[1:] - u2ij[:-1]) / dx
            + (ui2j2[1:-1, 1:] * vi2j2[1:-1, 1:] - ui2j2[1:-1, :-1] * vi2j2[1:-1, :-1])
            / dy
            + (p[1:] - p[:-1]) / dx
            - nu
            * (
    ...
    def animate(x, y, u, v, p, interval=1):
        ...
        anim = animation.FuncAnimation(
            fig, update, frames=u.shape[-1], interval=interval, blit=False
        )
        plt.show()
...
if __name__ == "__main__":
    ...
    y_channel = np.linspace(0, Ly, ny + 2)
    u_left_profile = np.maximum(24 * (1 - y_channel) * (y_channel - 0.5), 0)
    BC = {
        "u_top": 0,
        "u_bot": 0,
        "u_left": u_left_profile,
    ...
    xij, yij, xi2j2, yi2j2, dx, dy, y0, yp0, redundant_coordinates, F, animate = (
        generate_system(Lx, Ly, nx, ny, nu, BC)
    )
    ...
    for i in range(nt):
        u[:, :, i], v[:, :, i], p[:, :, i], ut[:, :, i], vt[:, :, i], pt[:, :, i] = (
            redundant_coordinates(sol_y[:, i], sol_yp[:, i])
        )
